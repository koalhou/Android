...
public class LoadingOverlay extends LinearLayout {
	...
	public LoadingOverlay(Context context, AttributeSet attrs) {
		...
		if (isInEditMode())
			return;
		LayoutInflater.from(context).inflate(R.layout.control_loadingoverlay,
				this, true);
		initViews();
		registerListener();
	...
	public void setCancelable(boolean canclable) {
		this.cancelable = canclable;
		btn_loading_cancel.setVisibility(this.cancelable ? View.VISIBLE
				: View.GONE);
	}
