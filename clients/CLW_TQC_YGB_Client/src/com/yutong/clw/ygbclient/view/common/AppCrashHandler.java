/*******************************************************************************
 * @(#)AppCrashHandler.java 2012-6-21 Copyright 2012 Neusoft Group Ltd. All
...
import com.yutong.clw.ygbclient.business.loginfo.ExceptionInfoBiz;
...
public final class AppCrashHandler implements UncaughtExceptionHandler
{
    ...
    private boolean handleException(final Throwable ex)
    {
        ...
        ex.printStackTrace();
        // 1.获取当前程序的版本号. 版本的id
        String versioninfo = SysInfoGetUtil.getPackageInfo(this.context);
        // 2.获取手机的硬件信息.
        String mobileInfo = getMobileInfo();
        // 4.把所有的信息 还有信息对应的时间 提交到服务器
        final String txt = String.format("程序版本号:%s;手机硬件信息：%s；", versioninfo, mobileInfo);
        Logger.e(AppCrashHandler.class, "[自定义异常类]:日期：", dataFormat.format(new Date()), "程序版本号：", versioninfo, "手机硬件信息：", mobileInfo,
                "未捕获的异常信息：", ex);
        // 使用Toast来显示异常信息
        new Thread()
        {
            ...
            public void run()
            {
                ...
                try
                {
                    String msg = txt + Logger.getMsg(ex);

                    (new ExceptionInfoBiz(AppCrashHandler.this.context, msg)).reportExceptionInfo();
                }
