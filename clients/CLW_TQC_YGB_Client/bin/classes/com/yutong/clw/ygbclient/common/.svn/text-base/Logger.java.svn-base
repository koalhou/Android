...
public final class Logger
{
    ...
    /**
     * 获取异常的字符串信息
     *@author zhangyongn 2013-11-19 下午1:49:13
     *
     * @param er
     * @return
     */
    public static String getMsg(Throwable er)
    {
        if (er == null)
            return "";
    ...
    private static void saveFile(String Level, String className, String logMsg)
    {
        ...
        if (isCanWriteLog)
        {
            ...
            try
            {
                ...
                String str = new StringBuilder().append("[").append(getCurrentTime()).append("]").append("--").append("[").append(Level)
                        .append("]").append("--").append("[Thread-").append(Thread.currentThread().getId())
                        .append(getNul(String.valueOf(Thread.currentThread().getId()), 4)).append("]").append("--").append("--<")
                        .append(className).append(getNul(className, 70)).append(">:").append(logMsg).append("\n").toString();
    ...
    public static String getLogFilePath(){
    	return   logFileLocation + logName;
    }
    ...
    {
        String fileLocStr = getProperty("LOG_FILE_LOCATION", Environment.getExternalStorageDirectory().getAbsolutePath() + "/").trim();
