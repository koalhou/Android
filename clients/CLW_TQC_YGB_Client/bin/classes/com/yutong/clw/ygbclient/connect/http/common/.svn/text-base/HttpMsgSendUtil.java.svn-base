...
public final class HttpMsgSendUtil
{
    ...
    private static HttpRes sendPostMsg(AbstractReq msg, int sendCount, int maxSendCount)
    {
        ...
        if (NetworkCheckUitl.isOnline())
        {
            ...
            try
            {
                ...
                Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：POST消息第", sendCount, "次发送，" + "Request URI：", uri, "，access_token:",
                        msg.getAccessToken(), "，If-None-Match:", msg.getIfNoneMatch(), "，MsgBody：", msgBody);
                ...
                Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：POST消息响应，statusCode：", httpRes.getStatusCode(), "，content：",
                        httpRes.getContent(), "，requestUri：", msg.getReuqestUrl());
                return httpRes;
            }
            catch (Exception e)
            {
    ...
    {
        return sendGetMsg(msg, 1, SysConfigUtil.getHttpMaxResendCount());
    }

    private static HttpRes sendGetMsg(AbstractReq msg, int sendCount, int maxSendCount)
    {
        HttpRes httpRes = new HttpRes();
        if (NetworkCheckUitl.isOnline())
        {
            ...
            try
            {
                ...
                Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：GET消息第", sendCount, "次发送，Request URI：", uri, "，access_token:",
                        msg.getAccessToken(), "，If-None-Match:", msg.getIfNoneMatch());
            ...
            catch (Exception e)
            {
                ...
                if (tempSendCount <= maxSendCount)
                {
                    return sendGetMsg(msg, tempSendCount, maxSendCount);
                }
                else
                {
                    httpRes.setException(true);
    ...
    private static HttpRes sendPutMsg(AbstractReq msg, int sendCount, int maxSendCount)
    {
        ...
        if (NetworkCheckUitl.isOnline())
        {
            ...
            try
            {
                ...
                Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：PUT消息第", sendCount, "次发送，Request URI：", uri, "，access_token:",
                        msg.getAccessToken(), "，If-None-Match:", msg.getIfNoneMatch(), "，MsgBody：", msgBody);
    ...
    private static HttpRes sendDeleteMsg(AbstractReq msg, int sendCount, int maxSendCount)
    {
        ...
        if (NetworkCheckUitl.isOnline())
        {
            ...
            try
            {
                ...
                Logger.i(HttpMsgSendUtil.class, "[HTTP REST消息发送工具类]：DELETE消息第", sendCount, "次发送，Request URI：", uri, "，access_token:",
                        msg.getAccessToken(), "，If-None-Match:", msg.getIfNoneMatch());
    ...
    private static void setBizHeader(AbstractReq msg, HttpRequestBase httpClient)
    {
        ...
        if (SysConfigUtil.getCacheOpen() && msg.getIfNoneMatch() != null)
        {
