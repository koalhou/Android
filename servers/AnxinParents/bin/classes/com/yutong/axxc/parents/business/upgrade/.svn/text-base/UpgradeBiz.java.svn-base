...
package com.yutong.axxc.parents.business.upgrade;
...
/**
 * 升级业务逻辑类
 * @author zhangzhia 2013-8-22 上午10:26:27
 *
 */
public class UpgradeBiz extends YtAsyncTask {
     ...
     * @param url 下载地址
     */
    public UpgradeBiz(Context context, Handler handler, String url) {
    ...
    private void install(Context context) {
        Logger.i(this.getClass(), logTypeName + "安装新版软件");
    ...
    private boolean downFile(String downloadUrl, Handler handler) {
        Logger.i(this.getClass(), logTypeName + "下载新版软件");
        ...
        try {
            ...
            if (stattusCode >= 200 && stattusCode < 300) {
                ...
                File tempFile = new File(Environment.getDownloadCacheDirectory(), UPDATE_SAVENAME);
                if (tempFile.exists()) {
                    tempFile.delete();
                }
                ...
                while (!isCanceled() && (byteCount = inputStream.read(buffer)) != -1) {
                    ...
                    Logger.d(this.getClass(), logTypeName + "新版软件下载中，rate:", rate);
                    outputStream.write(buffer, 0, byteCount);
                }
                Logger.d(this.getClass(), logTypeName + "新版软件下载完成");
            ...
            Logger.e(this.getClass(), logTypeName + "发送报文异常，详细信息:", e);
