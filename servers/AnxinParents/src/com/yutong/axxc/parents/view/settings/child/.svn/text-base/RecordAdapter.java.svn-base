...
import com.yutong.axxc.parents.view.home.LocationMapActivity;
...
public class RecordAdapter extends BaseAdapter {
	...
	private View.OnClickListener curgpsbtnlistener = new curgpsBtnOnOkListener();
	...
	public RecordAdapter(Context context, List<PushMessage> messages) {
		super();
		this.context = context;
		this.msgs = messages;
	...
	private void updateValue(int type, ViewHolder holder, PushMessage m) {
		try {
			...
			default:
				...
				holder.gpsbtn.setVisibility(m.isShowGps() ? View.VISIBLE
						: View.GONE);
				String t=StringUtil.isNull(m.getStationname())?"无站点名称":m.getStationname();				
	...
	private class curgpsBtnOnOkListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			Toast.makeText(
					YtApplication.getInstance().getApplicationContext(),
					"current gps：" + m.getBody(), Toast.LENGTH_SHORT)
					.show();
		}
	}
