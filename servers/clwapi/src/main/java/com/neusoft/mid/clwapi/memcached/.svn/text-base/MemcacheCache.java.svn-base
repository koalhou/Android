 ...
 */
package com.neusoft.mid.clwapi.memcached;
...
public class MemcacheCache implements Cache {
	...
	public void put(Object key, Object value) {
		...
		if (null == key || "".equals(key)) {
			...
			try {
				...
				if (null != value) {
					if (value instanceof MobileUsrAllInfo) {
						logger.info("即将被缓存的数据类型为MobileUsrAllInfo");
						MobileUsrAllInfo userInfo = (MobileUsrAllInfo) value;
						if (userInfo.getValidSeconds() > 0) {
							cacheSetFlg = this.client.set(setKey, userInfo.getValidSeconds(), value);
						} else {
							logger.debug("用户token已过期,不缓存用户数据");
							MDC.remove("SUB_ID");
							return;
						}
					} else if(value instanceof WeatherInfoResp){
						logger.info("即将被缓存的数据类型为WeatherInfoResp");
						WeatherInfoResp weather = (WeatherInfoResp) value;
						if (weather.getValidSeconds() > 0) {
							cacheSetFlg = this.client.set(setKey, weather.getValidSeconds(), value);
						} else {
							logger.debug("用户token已过期,不缓存用户数据");
							MDC.remove("SUB_ID");
							return;
	...
	private String getCacheKey(Object key) {
		if (null == key) {
			...
			return CachedCommon.CACHED_TOKEN_KEY_PRE + (String) key;
		} else {
			return CachedCommon.CACHED_TOKEN_KEY_PRE + key.toString();
		}
	}
